...
%{__python2} -m tox -e py27 -- --skip-servertests
%if 0%{?with_python3}
%{__python3} -m tox -e py35 -- --skip-servertests
%endif
...
install -t "%{buildroot}/%{_sysconfdir}/custodia" contrib/config/custodia.conf
install -t "%{buildroot}/%{_unitdir}" contrib/config/custodia.socket contrib/config/custodia.service
